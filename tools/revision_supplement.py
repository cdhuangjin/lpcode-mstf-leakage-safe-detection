...
def run():
    ...
    s+='\n\n# S6 Interpretation and access\n\nAll five quantitative transformations are retained; combined applies removal, renaming and formatting, without injection. Parsing checks do not prove semantic equivalence. Per-language transformed scores, change fractions and parser-regression statistics remain in the accompanying attack decomposition CSV. Full grouped importance and ranking stability are available in the local mechanism CSVs. These local materials are not represented as a completed public release.\n\nTODO_PUBLIC_RELEASE remains a submission blocker. The repository requires permitted source code, licence, pinned environment, manifests, aggregate records, figure data and a versioned release. No repository push, licence selection or archival deposit was performed by this manuscript revision.\n'
