...
def generate():
    ...
    refs+=extra
    release_ref=OUT/'release_reference.json'
    if release_ref.exists():refs.append(json.loads(release_ref.read_text(encoding='utf-8')))
    ...
    report='# Reference verification audit\n\nChecked 5 September 2026. Twenty-one literature records plus one software/evidence deposit; primary metadata stored with this revision. ArXiv citations identify the consulted record, not a claim that no published version exists. No venue, DOI or publication date is inferred from an unverified secondary citation. Complete authors are retained in the data and bibliography. RoPGen author metadata is corrected against the primary author display because its machine-readable list splits one name. Original LPcode, CodeMirage, Rahman and language-agnostic entries are not copied from the old bibliography.\n\n'+table(['No.','Verified title','Cited year','Cited source/status','DOI','arXiv','Primary source'],audit)+'\n\nEvery item is cited in the revised text. The new deposit record is verified by anonymous GitHub access; the separate release-stage bibliography reports distinguish single-source checks from multi-source agreement. Preprint versions may differ from older draft metadata; the source HTML snapshot identifies what was consulted. Conference status is asserted only for primary sources recorded above. Optional final copy-edit: replace other arXiv citations with publisher versions after checking matching text and metadata; do not fabricate a venue.\n'
